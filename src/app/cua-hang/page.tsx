  ...
  return (
    <main className={`${lora.className} bg-white text-[#2b2b2b]`}>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 mb-6 flex items-center justify-between text-sm text-gray-600 border-b border-gray-200 pb-4">
        ...
        <div className="flex items-center gap-4 text-gray-700">
          <div className="hidden sm:flex items-center space-x-2">
            ...
            {[12, 15, 30, "Tất cả"].map((n) => (
              <button
                key={n}
                onClick={() =>
                  n === "Tất cả"
                    ? setShowCount(products.length)
                    : setShowCount(Number(n))
                }
                className={`pb-[2px] ${
                  showCount === n ||
                  (n === "Tất cả" && showCount === products.length)
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 pb-16 grid grid-cols-1 lg:grid-cols-4 gap-8 sm:gap-10">
        <aside
          className={`fixed lg:static top-0 left-0 z-50 lg:z-auto w-4/5 sm:w-2/3 lg:w-auto h-full lg:h-auto bg-white border-r border-gray-200 shadow-lg lg:shadow-none p-6 lg:p-0 overflow-y-auto transition-transform duration-300 ${
                  ...
                  {group.items.map((item, j) => (
                    <li key={j}>
                      <Link
                        ...
                        onClick={() => setShowSidebar(false)}
                        className="block py-1 pl-2 border-l border-[#d4b48a] text-[14px] text-gray-600 hover:text-[#b5895b] transition-all"
                      >
                        {item.name}
                      </Link>
                    </li>
          ...
          {products.length === 0 ? (
            ...
            >
              {products.slice(0, showCount).map((item) => (
                <Link
                  key={item.id}
                  href={`/cua-hang/san-pham/${item.id}`}
